...
export function useRegisterDevice({ onSuccess } = {}) {
  ...
  const saveDeviceLocal = (device) => {
    try {
      if (device?.device_id) localStorage.setItem("device_id", device.device_id);
      if (device?.device_name) localStorage.setItem("device_name", device.device_name);
      if (device?.token) localStorage.setItem("device_token", device.token);
    } catch {}
  };
  ...
  const setTokenCookie = (token) => {
    if (!token) return;
    try {
      document.cookie = `device_token=${encodeURIComponent(token)}; path=/; SameSite=Lax`;
    } catch {}
  };
